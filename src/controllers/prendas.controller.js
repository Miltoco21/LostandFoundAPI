...
export const getPrendasByRut = async (req, res) => {
  try {
    console.log("🎯 getPrendasByRut INICIANDO - Ruta: /prendas/buscar");
    ...
    if (!rut) {
      ...
      return res.status(400).json({ 
        ...
        example: "GET /prendas/buscar?rut=17951288-2",
        received: req.query,
        hint: "Asegúrate de incluir ?rut=TU_RUT en la URL"
      });
    }
    ...
    // Si no se encontraron resultados, informar claramente
    if (results.length === 0) {
      console.log(`ℹ️ No se encontraron prendas para RUT: ${rut}`);
    ...
    console.log("📤 Enviando respuesta con", results.length, "prendas");
    res.status(200).json({
