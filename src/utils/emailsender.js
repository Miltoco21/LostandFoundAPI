...
// emailsender.js - Sistema de plantillas dinámicas por estado - VERSIÓN RESEND
import { Resend } from 'resend';

// Configuración de Resend - Compatible con Render Free (HTTP API)
const resend = new Resend(process.env.RESEND_API_KEY);

// Plantillas de email por estado - IDÉNTICAS A TU VERSIÓN ORIGINAL
const emailTemplates = {
  "Encontrada - Pendiente de devolución": {
    subject: "¡Tu prenda ha sido encontrada! 🎉",
    template: (garmentDetails) => `
...
// Función principal para enviar emails dinámicos - ADAPTADA PARA RESEND
export const sendStatusUpdateEmail = async (userEmail, garmentDetails, newStatus) => {
  console.log("🔍 ========== INICIO sendStatusUpdateEmail (RESEND) ==========");
  console.log(`📧 Email destino: ${userEmail}`);
  console.log(`📊 Estado solicitado: "${newStatus}"`);
  console.log(`📦 Detalles prenda:`, garmentDetails);
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log("❌ Variable de entorno RESEND_API_KEY no configurada");
  ...
  try {
    console.log("🚀 Enviando email via Resend HTTP API...");
...
// Función para testing - verificar configuración de Resend
export const testEmailConfiguration = async () => {
  ...
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      return { 
        configured: false, 
        error: "Variable RESEND_API_KEY no configurada",
        provider: 'resend'
      };
    }
    ...
    if (!process.env.RESEND_API_KEY.startsWith('re_')) {
      return {
        configured: false,
        error: "API key de Resend debe comenzar con 're_'",
        provider: 'resend'
      };
    }

    // Intentar enviar un email de prueba (comentado por defecto)
...
// Función de prueba rápida - envía a milton206@gmail.com
export const sendTestEmail = async () => {
  console.log("🧪 Enviando email de prueba a milton206@gmail.com...");
  
  const testGarment = {
    id: 'TEST-001',
    tipo_prenda: 'Sudadera Nike',
    talla: 'M',
    estado: 'Encontrada',
    observaciones: 'Email de prueba del sistema'
  };
  
  return await sendStatusUpdateEmail(
    'milton206@gmail.com',
    testGarment,
    'Encontrada - Pendiente de devolución'
  );
